  ...
  private class FindCaseDocumentViewsList extends AbstractList<Object[]>
  {
    ...
    public Object[] get(int index)
    {
      ...
      else
      {
        ...
        Vector row = (Vector) results.get(index);
        DBCase dbCase = new DBCase();
        dbCase.setCaseId((String) row.get(0));
        dbCase.setCaseTypeId((String) row.get(1));
        dbCase.setCaseTypeNum((String) row.get(2));
        dbCase.setSdetext((String) row.get(3));
        dbCase.setRegistryDate((String) row.get(4));
        dbCase.setRegistryTime((String) row.get(5));
        dbCase.setStdugr((String) row.get(6));
        dbCase.setStdumod((String) row.get(7));
        dbCase.setStddgr((String) row.get(8));
        dbCase.setStddmod((String) row.get(9));
        dbCase.setStdhgr((String) row.get(10));
        dbCase.setStdhmod((String) row.get(11));
        result[0] = dbCase;

        DBCaseDocument dbCaseDocument = new DBCaseDocument();
        dbCaseDocument.setAplcod((String) row.get(12));
        dbCaseDocument.setDocorigen((String) row.get(13));
        dbCaseDocument.setDoccod((String) row.get(14));
        dbCaseDocument.setModelcod((String) row.get(15));
        dbCaseDocument.setDocnompc((String) row.get(16));
        dbCaseDocument.setIdentificador((String) row.get(17));
        dbCaseDocument.setDocId(String.valueOf(row.get(18)));
        dbCaseDocument.setCaseId(row.get(19) != null ? String.valueOf(row.get(19)) : null);
