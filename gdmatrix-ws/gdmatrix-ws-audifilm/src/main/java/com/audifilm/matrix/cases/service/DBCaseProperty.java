...
public enum DBCaseProperty
{
  ...
  static protected  List<Property> loadCaseExtendedProperties(EntityManager entityManager, DBCase dbCase)
  {
    ...
    Query queryResponsable = entityManager.createNamedQuery("findCaseState");
    queryResponsable.setFirstResult(0);
    queryResponsable.setMaxResults(0);
    queryResponsable.setParameter("caseId", dbCase.getCaseId());

    List<DBCaseState> resultList = queryResponsable.getResultList();

    Property propResponsables = new Property();
    propResponsables.setName("responsables");

    int respIndex=0;
    for(DBCaseState caseState:resultList)
    {
    ...
    queryResponsable = entityManager.createNamedQuery("findCaseStateResponsible");
    queryResponsable.setFirstResult(0);
    queryResponsable.setMaxResults(0);
    queryResponsable.setParameter("caseId", dbCase.getCaseId());

    List<DBCaseStateResponsible> resultListResponsible = queryResponsable.getResultList();
    for(DBCaseStateResponsible stateResponsible: resultListResponsible)
    {
      propResponsables.getValue().add(
              String.format("%04d;%s", ++respIndex,stateResponsible.getPropertyValue()));
    }
    resultList.clear();
