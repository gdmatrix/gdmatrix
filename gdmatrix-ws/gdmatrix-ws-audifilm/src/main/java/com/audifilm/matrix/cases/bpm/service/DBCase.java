...
public class DBCase extends DBGenesysEntity
{
  ...
  public void setCaseState(DBCaseState caseState) {
  ...
  public void copyTo(WSEndpoint endpoint, Case exp)
  {
    ...
    if (caseState!=null) {
      exp.setState(caseState.getDescription());
      exp.getProperty().addAll(DBCaseProperty.getPropertyList(caseState.getClass(), caseState));

      if (caseState.getStateId().equals("9999")) {
        exp.setEndDate(caseState.getEndDate());
        exp.setEndTime(caseState.getEndTime());
      }
    }
  }
