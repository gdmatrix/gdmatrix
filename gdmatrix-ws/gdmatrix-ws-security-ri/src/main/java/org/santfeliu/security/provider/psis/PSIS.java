...
public class PSIS implements SecurityProvider
{
  ...
  public boolean validateSignatureCMS(byte[] encoded, OutputStream out)
  {
    try
    {
      ...
      variables.put("SIGNATURE", Base64.encode(encoded));
      ...
      return resultMinor != null && resultMinor.indexOf(":valid:") != -1;
    }
    catch (Exception ex)
    {
    ...
    try
    {
      ...
      if (certEncoded == null)
      {
        try
        {
          ...
          certEncoded = Base64.decode(certBase64);
        }
        catch (Exception ex)
        {
    ...
    String digestMethod, byte[] certEncoded)
  {
    try
    {
      ...
      variables.put("DIGEST_VALUE", Base64.encode(digest));
      variables.put("CERTIFICATE", Base64.encode(certEncoded));
      String message = Template.create(template).merge(variables);
  ...
  private void testCreateTimeStamp() throws Exception
  {
    ...
    /*
     byte[] tst = createCMSTimeStamp(digest, 
      "http://www.w3.org/2000/09/xmldsig#sha1", null);
    System.out.println(Base64.encode(tst));
    */
  ...
  public static void main(String args[])
  {
    try
    {
      ...
      PSIS psis = new PSIS();
      File file = new File("c:/test1.p7m");
      byte[] data = IOUtils.toByteArray(new FileInputStream(file));
      boolean valid =
        psis.validateSignatureCMS(data, new FileOutputStream("c:/ticket.xml"));
