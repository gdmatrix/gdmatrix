...
public abstract class ApplyPoliciesTask extends WSTask
{
  ...
  protected String evalActivationDate(
    Case _case, Document document, Policy policy, Object currentPolicyViews)
  {
      ...
      activationDateExpression.trim().length() > 0)
    {
      ...
      try
      {
        Scriptable scriptable = getScriptable(cx);
        ...
        if (currentPolicyViews != null)
        {
          scriptable.put("currentPolicies", scriptable, currentPolicyViews);
        }
