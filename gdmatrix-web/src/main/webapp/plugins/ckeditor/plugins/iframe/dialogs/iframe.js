      ...
      if (isCheckbox)
        ...
        if (this.id == 'width'||this.id == 'height') {
          if (value.indexOf('px') < 0 
              ...
              && value.indexOf('%') < 0)
            value = value + "px";
          var style = iframeNode.getStyle(this.id);
          if (style == null || style == '')
            iframeNode.setStyle(this.id, value);
        }
        this.setValue(value);
      }
    }
    else if (this.id == 'width'||this.id == 'height'){
      var style = iframeNode.getStyle(this.id);
      if (style != null && style != '')
        this.setValue(style);
    }
  ...
  CKEDITOR.dialog.add('iframe', function (editor) {
    ...
    return {
      ...
      onOk: function () {
        ...
        iframeNode.setStyle("width", iframeNode.getAttribute("width"));
        iframeNode.setStyle("height", iframeNode.getAttribute("height"));
