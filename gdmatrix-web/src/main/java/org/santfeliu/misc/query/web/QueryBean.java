 ...
 * @author realor
 */
public class QueryBean extends FacesBean implements Savable, QueryFinder
{
  // Query Type
  ...
  public void loadQuery(String queryName) throws Exception
  {
    Document document = getDocumentManagerClient().
      loadDocumentByName(QUERY_TYPEID, QUERY_NAME_PROPERTY, queryName, null, -1);
  ...
  public void saveQuery() throws Exception
  {
    ...
    QuerySearchBean querySearchBean =
      (QuerySearchBean)getBean("querySearchBean");
    querySearchBean.clearDocuments();
  }
  ...
  {
    Document document = getDocumentManagerClientForAdmin().
      loadDocumentByName(QUERY_TYPEID, QUERY_NAME_PROPERTY, queryName, null, -1);    
    if (document == null) throw new Exception("QUERY_NOT_FOUND");
    return document.getContent().getData().getInputStream();
  }    

  private void clearBean(String beanName)
  {
