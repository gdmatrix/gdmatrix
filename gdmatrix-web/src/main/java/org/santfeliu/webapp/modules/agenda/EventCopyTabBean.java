...
public class EventCopyTabBean extends PageBean implements Serializable
{
  ...
  public void setMonthlyDayOfMonth(int monthlyDayOfMonth)
  {
    this.monthlyDayOfMonth = monthlyDayOfMonth;
  }

  public String getMonthlyDayOfWeekType()
  {
    return monthlyDayOfWeekType;
  }

  public void setMonthlyDayOfWeekType(String monthlyDayOfWeekType)
  {
    this.monthlyDayOfWeekType = monthlyDayOfWeekType;
  }

  public int getYearlyDayOfMonth()
  {
    return yearlyDayOfMonth;
  }

  public void setYearlyDayOfMonth(int yearlyDayOfMonth)
  {
    this.yearlyDayOfMonth = yearlyDayOfMonth;
  }

  public String getYearlyDayOfWeekType()
  {
    return yearlyDayOfWeekType;
  }

  public void setYearlyDayOfWeekType(String yearlyDayOfWeekType)
  {
    this.yearlyDayOfWeekType = yearlyDayOfWeekType;
  }
  ...
  public String getMonthlyWeekPosition()
  {
    return monthlyWeekPosition;
  }

  public void setMonthlyWeekPosition(String monthlyWeekPosition)
  {
    this.monthlyWeekPosition = monthlyWeekPosition;
  }

  public String getYearlyWeekPosition()
  {
    return yearlyWeekPosition;
  }

  public void setYearlyWeekPosition(String yearlyWeekPosition)
  {
    this.yearlyWeekPosition = yearlyWeekPosition;
  }
  ...
  public void setCheckAttendantsAvailability(
    boolean checkAttendantsAvailability)
  {
  ...
  {
    setFrequencyMode(getSelectedIndex());
  }
  ...
  public EventRow duplicate(long timeOffset, long duration) throws Exception
  {
    ...
    //set parameters
    setFrequencyMode(EventCopyTabBean.DAYLY);
    setDaylyFrequencyMode(EventCopyTabBean.BY_FREQUENCY_VALUE);
    setDaylyFrequencyValue(1);
    setEndRangeMode(EventCopyTabBean.BY_MAX_ITERATIONS);
...
//    copyRecurrences(addMasterEventId);
   ...
   */
  public static long getTimeBetweenDates(String startDateTime,
    String endDateTime)
  {
  ...
  private long timeToLong(String duration)
  {
    ...
    if (!StringUtils.isBlank(duration))
    {
      if (duration.contains(DAYS_SELECTOR))
      {
        ...
        result = TimeUnit.MINUTES.convert(Long.parseLong(parts[0]),
          TimeUnit.DAYS);
        if (parts.length > 1)
          duration = parts[1];
        else
      ...
      if (duration.contains(HOURS_SELECTOR))
      {
        ...
        result = result + TimeUnit.MINUTES.convert(Long.parseLong(parts[0]),
          TimeUnit.HOURS);
        if (parts.length > 1)
          duration = parts[1];
        else
  ...
  public class EventRow implements Serializable
  {
    ...
    {
      String endDateTime = TextUtils.formatDate(date, "yyyyMMddHHmmss");
  ...
  private class HourPattern implements Serializable
  {
    ...
    public void setDuration(long duration)
    {
      ...
      if (duration > time)
      {
        ...
        long t = sDate.getTime() + TimeUnit.MILLISECONDS.convert(duration,
          TimeUnit.MINUTES);
  ...
  private abstract class PreviewListBuilder
  {
    ...
    protected Event copyEvent(Date date, HourPattern hourPattern)
    {
      ...
      long endMillis =
        TextUtils.parseInternalDate(startDateTime).getTime() + duration * 60000;
      e.setEndDateTime(TextUtils.formatDate(new Date(endMillis),
        "yyyyMMddHHmmss"));
      ...
      HourPattern hourPattern)
    {
        ...
        && !StringUtils.isBlank(hourPattern.getFormattedEndHour()))
      {
        ...
        boolean inRange =
          scal.getTimeInMillis() + hourPattern.getDurationInMillis() <=
          ecal.getTimeInMillis();
        if (!inRange)
        {
          error("OUT_OF_RANGE");
          return false;
  ...
  private class MonthlyPreviewListBuilder extends PreviewListBuilder
  {
    ...
    public boolean addEvents(List<EventRow> events, Calendar cal)
    {
      ...
      {
        if (monthlyFrequencyValue0 == 0 || monthlyDayOfMonth == 0)
          return false;

        if (monthlyDayOfMonth == cal.get(Calendar.DAY_OF_MONTH))
        {
          result = addEvent(events, cal.getTime(), hourPattern);
        }

        cal.add(Calendar.MONTH, monthlyFrequencyValue0);
        cal.set(Calendar.DAY_OF_MONTH, monthlyDayOfMonth);
      }
      else
      {
        if (monthlyWeekPosition == null || monthlyDayOfWeekType == null)
          return false;
        ...
        int day = 1;
        int position = Integer.valueOf(monthlyWeekPosition);
        int counter = position;

        while (counter > 0 && month == cal.get(Calendar.MONTH))
        {
          if (monthlyDayOfWeekType.equals("8")) //Day
          {
            day = cal.get(Calendar.DAY_OF_MONTH);
            if (position != 5)
              counter--;
          }
          else if (monthlyDayOfWeekType.equals("9")) //Working day
          {
            int dayOfWeek = cal.get(Calendar.DAY_OF_WEEK);
          ...
          else if (monthlyDayOfWeekType.equals("10")) //Weekend day
          {
            int dayOfWeek = cal.get(Calendar.DAY_OF_WEEK);
          ...
          {
            int dayOfWeek = cal.get(Calendar.DAY_OF_WEEK);
            if (dayOfWeek == Integer.valueOf(monthlyDayOfWeekType).intValue())
            {
              day = cal.get(Calendar.DAY_OF_MONTH);
              if (position != 5)
                counter--;
  ...
  private class YearlyPreviewListBuilder extends PreviewListBuilder
  {
    ...
    public boolean addEvents(List<EventRow> events, Calendar cal)
    {
      ...
      {
        if (month0 == null || yearlyDayOfMonth == 0)
          return false;

        int month = Integer.valueOf(month0).intValue();
        if (yearlyDayOfMonth == cal.get(Calendar.DAY_OF_MONTH) &&
        ...
        cal.set(Calendar.DAY_OF_MONTH, yearlyDayOfMonth);
        cal.set(Calendar.MONTH, month);
      }
      else
      {
        if (yearlyWeekPosition == null || yearlyDayOfWeekType == null ||
          month1 == null)
          return false;
        ...
        int day = 1;
        int position = Integer.valueOf(yearlyWeekPosition);
        int counter = position;

        while (counter > 0 && month == cal.get(Calendar.MONTH))
        {
          if (yearlyDayOfWeekType.equals("8")) //Day
          {
            day = cal.get(Calendar.DAY_OF_MONTH);
            if (position != 5)
              counter--;
          }
          else if (yearlyDayOfWeekType.equals("9")) //Working day
          {
            int dayOfWeek = cal.get(Calendar.DAY_OF_WEEK);
            if (dayOfWeek >= Calendar.MONDAY && dayOfWeek <= Calendar.FRIDAY)
            {
          ...
          else if (yearlyDayOfWeekType.equals("10")) //Weekend day
          {
            int dayOfWeek = cal.get(Calendar.DAY_OF_WEEK);
            if (dayOfWeek == Calendar.SATURDAY || dayOfWeek == Calendar.SUNDAY)
            {
          ...
          {
            int dayOfWeek = cal.get(Calendar.DAY_OF_WEEK);
            if (dayOfWeek == Integer.valueOf(yearlyDayOfWeekType).intValue())
            {
              day = cal.get(Calendar.DAY_OF_MONTH);
              if (position != 5)
                counter--;
