...
@Named
@RequestScoped
public class EventMeetingFinderBean extends WebBean implements Serializable
{
  private static final String DEFAULT_RANGE_START_TIME_PROPERTY =
    "defaultRangeStartTime"; //HHmm format
  private static final String DEFAULT_RANGE_END_TIME_PROPERTY =
    "defaultRangeEndTime"; //HHmm format
  ...
  private static final String DEFAULT_EVENT_TYPE_ID_PROPERTY =
    "defaultEventTypeId";
  private static final String DEFAULT_ATTENDANT_TYPE_ID_PROPERTY =
    "defaultAttendantTypeId";
  private static final String DEFAULT_PAGE_SIZE_PROPERTY = "defaultPageSize";
  ...
  {
    return personTypeBean.getSelectItems();
  }  
  ...
  public void setRangeEndDateTime(String rangeEndDateTime)
  {
    if (StringUtils.isBlank(rangeEndDateTime))
    {
      ...
      this.rangeEndDateTime = 
        TextUtils.formatDate(calendar.getTime(),  "yyyyMMdd") +
        DEFAULT_RANGE_END_TIME + "59";
    }
    else
      this.rangeEndDateTime = 
  ...
  public String getAddedRoomId()
  {
    return addedRoomId;
  }

  public void setAddedRoomId(String addedRoomId)
  {
  ...
  {
    String pageSize =
      getSelectedMenuItem().getProperty(DEFAULT_PAGE_SIZE_PROPERTY);
    if (pageSize != null)
    {
      try
      {
  ...
  public Collection<Room> getRooms() 
  {
    if (rooms == null || rooms.isEmpty())
    {
      ...
      try
      {
        List<String> favRoomIds = userPreferences.getPreferences("Room");
        for (String favRoomId : favRoomIds)
        {
          Room roomView = new Room();
          roomView.setRoomId(favRoomId);
          roomView.setDescription(roomTypeBean.getDescription(favRoomId));
  ...
  public String show()
  {
    ...
    String defaultRangeStartTime =
      getProperty(DEFAULT_RANGE_START_TIME_PROPERTY);
    ...
    Calendar calendar = Calendar.getInstance();
    String today = TextUtils.formatDate(calendar.getTime(), "yyyyMMdd") +
      defaultRangeStartTime + "00";
    calendar.add(Calendar.DATE, 7);
    String weekAgo = TextUtils.formatDate(calendar.getTime(),  "yyyyMMdd") +
      defaultRangeEndTime + "59";
  ...
  {
    return "/pages/agenda/event_meeting_finder.xhtml";
  }
  ...
  public String addAttendant(String personId)
  {
    ...
    {
      String fullName = personTypeBean.getDescription(personId);
  ...
  public String addRoom(String roomId)
  {
    ...
    {
      String description = roomTypeBean.getDescription(roomId);
      Room room = new Room();
  ...
  public String findFreeSlots()
  {
    ...
    try
    {
      ...
      AgendaManagerClient port = AgendaModuleBean.getClient();
      ...
      {
        long startTime = 
          TextUtils.parseInternalDate(event.getStartDateTime()).getTime() -
          dtData.getTimePaddingInMillis();
        long endTime = 
          TextUtils.parseInternalDate(event.getEndDateTime()).getTime() +
          dtData.getTimePaddingInMillis();
      ...
      if (!personIds.isEmpty())
      {
        try
        {
          ...
          {
            long startTime =
              TextUtils.parseInternalDate(absence.getStartDateTime()).getTime();
            long endTime =
              TextUtils.parseInternalDate(absence.getEndDateTime()).getTime();
            board.addBusySlot(startTime, endTime);
          }
        }
        catch (Exception ex)
      ...
      while (cal.getTime().before(dEndDate))
      {
        ...
        //Add 1 millisecond to leave space for last slot
        t1 = cal.getTimeInMillis() + dtData.getEndHourInMillis() + 1;
        t2 = cal.getTimeInMillis() +
          TimeUnit.MILLISECONDS.convert(1, TimeUnit.DAYS) - 1;
        board.addBusySlot(t1, t2);
      ...
      //Compute free slots
      if (!StringUtils.isBlank(duration) &&
        !StringUtils.isBlank(durationSelector))
      ...
      freeSlots = board.getFreeSlots(lstartTime, lendTime,
        dtData.getDurationInMillis(), dtData.getStepsTimeInMillis());
  ...
  public void createEvent()
  {
    ...
    boolean creatingEvent = (this.editingSlot != null &&
      this.editingSlot.equals(slot));
    if (!creatingEvent)
    {
      int roomCount =
        slot.getAvailableRooms() == null ? 0 : slot.getAvailableRooms().size();
      if (roomCount > 1)
        this.editingSlot = slot;        
      else 
      {
        Room room = (roomCount == 1 ? slot.getAvailableRooms().get(0) : null);
        doEventCreation(slot, this.attendants, room);
      }
    }
    else
    {
      doEventCreation(this.editingSlot, this.attendants, selectedRoom);
  ...
  private void doEventCreation(ScheduleSlot slot, List<Attendant> attendants,
    Room room)
  {
    ResourceBundle bundle = ResourceBundle.getBundle(
      "org.santfeliu.web.resources.MessageBundle", getLocale());
    ...
    try
    {
      ...
      String defaultEventTypeId = getProperty(DEFAULT_EVENT_TYPE_ID_PROPERTY);
      if (defaultEventTypeId == null)
        defaultEventTypeId = DictionaryConstants.EVENT_TYPE;
      event.setEventTypeId(defaultEventTypeId);
      ...
      {
        org.matrix.agenda.Attendant attendant =
          new org.matrix.agenda.Attendant();
        attendant.setPersonId(att.getPersonId());
        attendant.setEventId(event.getEventId());
        String defaultAttendantTypeId =
          getProperty(DEFAULT_ATTENDANT_TYPE_ID_PROPERTY);
        if (defaultAttendantTypeId == null)
          defaultAttendantTypeId = DictionaryConstants.ATTENDANT_TYPE;
        attendant.setAttendantTypeId(defaultAttendantTypeId);
        port.storeAttendant(attendant);
      }
      info(bundle.getString("ATTENDANTS_ADDED"),
        new Object[]{attendants.size()});
      ...
      try
      {
        ...
        mailDomains =
          getSelectedMenuItem().getMultiValuedProperty(MAIL_DOMAINS);
        if (mailSender != null && mailDomains != null && !mailDomains.isEmpty())
        {
          List<ScheduleSlot> auxFreeSlots = new ArrayList(freeSlots);
          auxFreeSlots.remove(slot);
          sendMail(event, auxFreeSlots, attendants, room);
      ...
      navigatorBean.show("sf:Event", event.getEventId());
    }
    catch (Exception ex)
    {
  ...
  private void sendMail(Event event, List<ScheduleSlot> slots,
    List<Attendant> attendants, Room room) throws Exception
  {
    List<String> emails = new ArrayList();
    KernelManagerPort port = KernelModuleBean.getPort(true);
    for (Attendant att : attendants)
    {
    ...
    {
      throw new Exception(bundle.getString("NO_VALID_RECIPIENTS"));
    }
  }
          ...
          List<Attendant> attendants, Room room, ResourceBundle bundle)
  {
    ...
    String html =  user + " " +
      ...
      bundle.getString("EMAIL_STARTDATE") + ": " + 
        TextUtils.formatInternalDate(event.getStartDateTime(),
          "dd/MM/yyyy HH:mm") + "<br>" +
      bundle.getString("EMAIL_ENDDATE") + ": " +   
        TextUtils.formatInternalDate(event.getEndDateTime(),
          "dd/MM/yyyy HH:mm") + "<br>" +
  ...
  private String encodeAlternatives(List<ScheduleSlot> slots)
  {
    ...
    for (int i = 0; i < loops; i++)
    {
      ...
      result += "<li>" + slot.getStartDateTime() + " - " +
        slot.getEndDateTime() + "</li>";
    }
    result += "</ul>";
    return result;
  }
  ...
  public class RangeDateTimeData implements Serializable
  {
    ...
    {     
      ...
      if (this.duration > time)
      {
        ...
        long t = sDate.getTime() +
          TimeUnit.MILLISECONDS.convert(this.duration, TimeUnit.MINUTES);
      ...
      if (this.startHour != null)
      {
        ...
        return 
          (TimeUnit.MILLISECONDS.convert(Long.valueOf(hour), TimeUnit.HOURS) +
           TimeUnit.MILLISECONDS.convert(Long.valueOf(minutes),
             TimeUnit.MINUTES));
      }
      return 0;
    }
    ...
    public void setFormattedEndHour(String endHour)
    {
      if (endHour != null && endHour.length() == 5 && endHour.contains(":"))
      {
        ...
        (endHour.length() == 1 || endHour.length() == 2))
      {
    ...
    {
      if (startHour != null && startHour.length() == 5 &&
        startHour.contains(":"))
      {
        ...
        (startHour.length() == 1 || startHour.length() == 2))
      {
    ...
    private long timeToLong(String duration)
    {
      ...
      if (!StringUtils.isBlank(duration))
      {
        if (duration.contains(HOURS_SELECTOR))
        {
          ...
          result = result +
            TimeUnit.MINUTES.convert(Long.parseLong(parts[0]), TimeUnit.HOURS);
          if (parts.length > 1)
            duration = parts[1];
          else
            duration = "0";
