...
public class JDateChooser extends JPanel implements ActionListener,
	...
	public void actionPerformed(ActionEvent e) {
        ...
        Frame owner = (Frame)SwingUtilities.getWindowAncestor(getParent());
        if (dialog == null)
        {
          dialog = new JDialog(owner, "Select date", true);
          dialog.getContentPane().add(jcalendar);
          dialog.setSize(200, 200);
        }
		int x = calendarButton.getWidth()
				- (int)dialog.getSize().getWidth();
		...
		jcalendar.setCalendar(calendar);
	        ...
	        dialog.setSize(200, 200);
                ...
                {
                  location.y = Toolkit.getDefaultToolkit().getScreenSize().height - 250; 
                }
