/*
 * GDMatrix
 *  
 * Copyright (C) 2020, Ajuntament de Sant Feliu de Llobregat
 *  
 * This program is licensed and may be used, modified and redistributed under 
 * the terms of the European Public License (EUPL), either version 1.1 or (at 
 * your option) any later version as soon as they are approved by the European 
 * Commission.
 *  
 * Alternatively, you may redistribute and/or modify this program under the 
 * terms of the GNU Lesser General Public License as published by the Free 
 * Software Foundation; either  version 3 of the License, or (at your option) 
 * any later version. 
 *   
 * Unless required by applicable law or agreed to in writing, software 
 * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT 
 * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. 
 *    
 * See the licenses for the specific language governing permissions, limitations 
 * and more details.
 *    
 * You should have received a copy of the EUPL1.1 and the LGPLv3 licenses along 
 * with this program; if not, you may find them at: 
 *    
 * https://joinup.ec.europa.eu/software/page/eupl/licence-eupl
 * http://www.gnu.org/licenses/ 
 * and 
 * https://www.gnu.org/licenses/lgpl.txt
 */
package org.santfeliu.util.script;

import java.util.Date;
import java.util.Iterator;
import java.util.Map;
import org.mozilla.javascript.Context;
import org.mozilla.javascript.NativeJavaObject;
import org.mozilla.javascript.ScriptRuntime;
import org.mozilla.javascript.Scriptable;
import org.mozilla.javascript.ScriptableObject;

/**
 *
 * @author realor
 */
public class DefaultScriptable extends ScriptableObject
{
  public DefaultScriptable(Context context)
  {
    // init standard objects
    ScriptRuntime.initStandardObjects(context, this, true);
    FunctionFactory.initFunctions(this);
  }

  @Override
  public String getClassName()
  {
    return "DefaultScriptable";
  }

  public void putAll(Map variables)
  {
    Iterator<Map.Entry> iter = variables.entrySet().iterator();
    while (iter.hasNext())
    {
      Map.Entry entry = iter.next();
      String name = String.valueOf(entry.getKey());
      Object value = entry.getValue();
      super.put(name, this, value);
    }
  }

  public Object toJavaPrimitive(Object object)
  {
    if (object instanceof Scriptable)
    {
      String className = ((Scriptable)object).getClassName();
      if (className.equals("Boolean"))
      {
        object = org.mozilla.javascript.Context.jsToJava(object, Boolean.class);
      }
      else if (className.equals("Number"))
      {
        object = org.mozilla.javascript.Context.jsToJava(object, Double.class);
      }
      else if (className.equals("String"))
      {
        object = object.toString();
      }
      else if (className.equals("Date"))
      {
        object = org.mozilla.javascript.Context.jsToJava(object, Date.class);
      }
      else if (object instanceof NativeJavaObject)
      {
        object = ((NativeJavaObject)object).unwrap();
      }
    }
    return object;
  }
}
